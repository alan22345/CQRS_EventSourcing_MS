...
public class RestoreReadDbController {
    ...
    public ResponseEntity<BaseResponse> restoreReadDb() {
        try{
            ...
            return new ResponseEntity<>(new BaseResponse("Restore request completed successfully"), HttpStatus.CREATED);
        } catch (IllegalStateException e) {
            logger.log(Level.WARNING, MessageFormat.format("Client made bad request - {0}.", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            var safeErrorMessage ="Error while processing request to open a new bank account for id - {0}";
